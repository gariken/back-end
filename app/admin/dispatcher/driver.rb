ActiveAdmin.register Driver, namespace: :dispatcher do
  ...
  action_item only: :show do
    link_to 'Удалить фото', "/dispatcher/drivers/#{driver.id}/remove_photo", method: :put, data: { confirm: 'Вы уверены, что хотите удалить фото?' }
  end

  action_item only: :show do
    link_to 'Удалить права', "/dispatcher/drivers/#{driver.id}/remove_license", method: :put, data: { confirm: 'Вы уверены, что хотите удалить права?' }
  end
  ...
  index do
    ...
    column :tariff_id do |driver|
      ...
      driver.tariff_id.nil? ? '' : link_to("#{tariff.name}", "/dispatcher/tariffs/#{tariff.id}")
    end
    column :confirmed
    column :rating
    actions
